...
var iluminacion = "LOW";
//var State = "Growth";
var modo_iluminacion = "Automatico";
var modo_ventilacion = "Automatico";
var State = "Blooming";
var receivedData = "";
var DataString = "";
...
io.on('connection', function(socket) {  
    ...
    console.log('Cliente ip '+clientIp+' modo ventilacion = '+modo_ventilacion);
    console.log('connection :', socket.request.connection._peername);
    io.sockets.emit('modo_ventilacion', modo_ventilacion);
    io.sockets.emit('modo_iluminacion', modo_iluminacion);
    socket.on('door', function(data){
      console.log('PUERTA = '+data);
      if(data == 'open'){
    ...
    socket.on('modo_iluminacion', function(data){
      console.log('S=> modo_iluminacion = '+data);
       modo_iluminacion = data;
    });
    socket.on('iluminacion', function(data){
      if(modo_iluminacion == "Manual"){
        Panel_led(data, State);
      }
    });
    socket.on('modo_ventilacion', function(data){
       modo_ventilacion = data;
       console.log('S=> modo_ventilacion = '+data);
    });
    socket.on('ventilacion', function(data){
      if(modo_ventilacion == "Manual"){
...
setInterval(function() {
  ...
  if(modo_iluminacion == "Automatico" && LDR_superior > 500 && (current_hour >= 0 && current_hour < 8)){

    led_state = "LOW";
     console.log("LED STATE LOW");
    Panel_led(led_state, State);
  }else if(modo_iluminacion == "Automatico" && LDR_superior < 100 && (current_hour < 0 ||  current_hour >= 8)) {
    led_state = "HIGH";
    console.log("LED STATE HIGH");
    Panel_led(led_state, State);
  }

  if(modo_ventilacion == "Automatico" && ventilador_derecho == "0" && (((current_minute >= 0 && current_minute < 15) || (current_minute >= 30 && current_minute < 45)) || temperatura >= 30)){
    ventilacion = "HIGH";
    console.log("Ventilador HIGH");
    Sistema_ventilacion(ventilacion);
  }else if(modo_ventilacion == "Automatico" && ventilador_derecho == "1" && ((current_minute >= 15 && current_minute < 30) || (current_minute >= 45 && current_minute < 60))){ 
    ventilacion = "LOW";
